...
// (eq::) — the owner only supplies a reader for its parameter source and the sample rate.
//
...
struct TraceSet
{
    static constexpr int maxBands = eq::EqEngine::kMaxBands;
    static constexpr int numLanes = eq::kNumLanes;
    ...
    template <class ReadBand>   // eq::BandParams readBand (int b)
        ...
        for (int b = 0; b < maxBands; ++b)
        {
            ...
            for (int L = 0; L < numLanes; ++L)
                designs[(size_t) b][(size_t) L] = (bands[(size_t) b].on && bands[(size_t) b].lanes[(size_t) L].on)
                                    ? eq::designBand (eq::laneView (bands[(size_t) b], (eq::Lane) L), dfs)
                                    : eq::BandDesign {};
        }
    }

    const eq::BandParams& param  (int b) const noexcept
    {
        assert (b >= 0 && b < maxBands);
        return bands[(size_t) b];
    }
    const eq::BandDesign& design (int b, int lane) const noexcept
    {
    ...
    double bandDb (int b, double f, int lane) const noexcept
    {
        ...
        for (int s = 0; s < d.n; ++s) h *= eq::evalCoeffs (d.sec[s], w);
    ...
    double compositeDbAxis (double f, eq::Axis a) const noexcept
    {
        const double dfs = designFs();
        const double w   = 2.0 * kPi * std::min (f, 0.499 * dfs) / dfs;
        const eq::Lane axl = eq::axisLane (a);
        const int axi = (int) axl;
        std::complex<double> h { 1.0, 0.0 };
        for (int b = 0; b < maxBands; ++b)
        {
            if (! bands[(size_t) b].on) continue;
            if (eq::laneActive (bands[(size_t) b], eq::Lane::Stereo))
                for (int s = 0; s < designs[(size_t) b][0].n; ++s) h *= eq::evalCoeffs (designs[(size_t) b][0].sec[s], w);
            if (a != eq::Axis::Stereo && eq::laneActive (bands[(size_t) b], axl))
                for (int s = 0; s < designs[(size_t) b][(size_t) axi].n; ++s) h *= eq::evalCoeffs (designs[(size_t) b][(size_t) axi].sec[s], w);
        }
        return 20.0 * std::log10 (std::max (1.0e-9, std::abs (h)));
    }
...
private:
    ...
    double fs = 44100.0;
    eq::BandParams bands[maxBands];
    eq::BandDesign designs[maxBands][numLanes];
};
