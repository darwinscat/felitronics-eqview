...
namespace felitronics::eqview { using felitronics::analysis::PlotMap; }
...
namespace felitronics::eqview::handles
{

//==============================================================================
// Filter-type behaviour under editing (core FilterType -> how the node/whiskers read).
inline bool hasGain (eq::FilterType t) noexcept
{
    return t == eq::FilterType::Bell     || t == eq::FilterType::LowShelf
        || t == eq::FilterType::HighShelf || t == eq::FilterType::Tilt;
}

inline bool isCut (eq::FilterType t) noexcept
{
    return t == eq::FilterType::HighPass || t == eq::FilterType::LowPass;
}

inline bool qRelevant (eq::FilterType t) noexcept   // Q sets bandwidth -> show the Q whiskers
{                                                    // HP/LP use discrete slopes (their own whisker), not Q
    return t == eq::FilterType::Bell      || t == eq::FilterType::BandPass
        || t == eq::FilterType::Notch     || t == eq::FilterType::AllPass
        || t == eq::FilterType::LowShelf  || t == eq::FilterType::HighShelf;   // resonant shelves have Q
}
...
inline bool slopeWhisker (eq::FilterType t) noexcept
{
    return isCut (t) || t == eq::FilterType::Notch || t == eq::FilterType::BandPass;
}

inline bool whiskerRelevant (eq::FilterType t) noexcept { return qRelevant (t) || isCut (t); }
...
inline bool restsOnZeroDb (eq::FilterType t) noexcept
{
    return isCut (t) || t == eq::FilterType::Notch || t == eq::FilterType::AllPass;
}
...
inline double whiskerBw (eq::FilterType t, double q, int slopeDb) noexcept
{
