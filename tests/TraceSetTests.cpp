...
static eq::BandParams bell (double freq, double gainDb, double q, eq::Lane lane = eq::Lane::Stereo)
{
    eq::BandParams bp;
    bp.on = true; bp.type = eq::FilterType::Bell;
    for (int L = 0; L < eq::kNumLanes; ++L) bp.lanes[(size_t) L].on = false;
...
int main()
{
    using TS = felitronics::eqview::TraceSet;
    ...
    {
        TS ts;
        ts.refresh (48000.0, [] (int b) { return b == 0 ? bell (1000.0, 6.0, 1.0) : eq::BandParams {}; });
    ...
    // the composite must equal the sum of the individual band curves at every frequency.
    {
        ...
        for (double f : { 50.0, 200.0, 700.0, 2500.0, 9000.0, 20000.0 })
        {
            ...
            check (nearEq (ts.compositeDbAxis (f, eq::Axis::Mid),
    ...
    // a domain lane folds ONLY into its own axis; the Stereo axis carries ST alone.
    {
        ...
        ts.refresh (48000.0, [] (int b)
        {
            if (b == 0) return bell (1000.0, 6.0, 1.0, eq::Lane::Stereo);
            if (b == 1) return bell (1000.0, 3.0, 1.0, eq::Lane::Left);
            return eq::BandParams {};
        });
        check (nearEq (ts.compositeDbAxis (1000.0, eq::Axis::Left),   9.0, 5e-3), "Left axis = ST + Left lane (6+3)");
        check (nearEq (ts.compositeDbAxis (1000.0, eq::Axis::Right),  6.0, 5e-3), "Right axis = ST only");
        check (nearEq (ts.compositeDbAxis (1000.0, eq::Axis::Mid),    6.0, 5e-3), "Mid axis = ST only");
        check (nearEq (ts.compositeDbAxis (1000.0, eq::Axis::Side),   6.0, 5e-3), "Side axis = ST only");
        check (nearEq (ts.compositeDbAxis (1000.0, eq::Axis::Stereo), 6.0, 5e-3), "Stereo axis carries ST alone");
    }
    ...
    // (-24.07 dB) — a real-fs design would flatten toward the 22.05k mirror and miss high.
    {
        ...
        {
            if (b != 0) return eq::BandParams {};
            eq::BandParams bp; bp.on = true; bp.type = eq::FilterType::LowPass;
            for (int L = 0; L < eq::kNumLanes; ++L) bp.lanes[(size_t) L].on = false;
    ...
    // A notch's null is bottomless in theory — the display floors it at exactly 20*log10(1e-9).
    {
        ...
        {
            if (b != 0) return eq::BandParams {};
            eq::BandParams bp; bp.on = true; bp.type = eq::FilterType::Notch;
            for (int L = 0; L < eq::kNumLanes; ++L) bp.lanes[(size_t) L].on = false;
    ...
    // product — the display must show -180, not -360.
    {
        ...
        check (nearEq (ts.compositeDbAxis (1000.0, eq::Axis::Mid), -180.0, 1e-3),
    ...
    // the design still exists for the dimmed ghost drawing.
    {
        ...
        check (nearEq (ts.compositeDbAxis (1000.0, eq::Axis::Mid), 6.0, 5e-3),
    ...
    {
        TS a, bset;
        auto reader = [] (int b) { return b == 0 ? bell (1000.0, 6.0, 1.0) : eq::BandParams {}; };
    ...
    // 4th-order Butterworth |H| = r^N / sqrt(1 + r^2N) at r = 1/2 -> -24.098 dB.
    {
        ...
        {
            if (b != 0) return eq::BandParams {};
            eq::BandParams bp; bp.on = true; bp.type = eq::FilterType::HighPass;
            for (int L = 0; L < eq::kNumLanes; ++L) bp.lanes[(size_t) L].on = false;
    ...
    // ripple is a coefficient-scaling defect.
    {
        ...
        {
            if (b != 0) return eq::BandParams {};
            eq::BandParams bp; bp.on = true; bp.type = eq::FilterType::AllPass;
            for (int L = 0; L < eq::kNumLanes; ++L) bp.lanes[(size_t) L].on = false;
            auto& ln = bp.lanes[0]; ln.on = true; ln.freq = 1000.0; ln.Q = 1.0; ln.slope = 12;
            return bp;
        });
    ...
    // antisymmetric, and the total span across the axis equals the gain parameter.
    {
        ...
        {
            if (b != 0) return eq::BandParams {};
            eq::BandParams bp; bp.on = true; bp.type = eq::FilterType::Tilt;
            for (int L = 0; L < eq::kNumLanes; ++L) bp.lanes[(size_t) L].on = false;
    ...
    // fc/Q is an exact reciprocal pair — the cascade must be 0 dB at EVERY frequency.
    {
        ...
        bool flat = true;
        for (double f = 20.0; f < 40000.0; f *= 1.5) flat = flat && nearEq (ts.compositeDbAxis (f, eq::Axis::Mid), 0.0, 2e-3);
    ...
    {
        auto shelfAt = [] (eq::FilterType t, double fc, double g)
        {
            TS ts;
            ts.refresh (48000.0, [&] (int b)
            {
                if (b != 0) return eq::BandParams {};
                eq::BandParams bp; bp.on = true; bp.type = t;
                for (int L = 0; L < eq::kNumLanes; ++L) bp.lanes[(size_t) L].on = false;
    ...
    // while its own curve stays designed — and the two bypass LEVELS gate independently.
    {
        ...
        check (nearEq (ts.compositeDbAxis (1000.0, eq::Axis::Mid), 0.0, 1e-9),
    ...
    // Left-lane test cannot distinguish from "correctly ST-only".
    {
        ...
        ts.refresh (48000.0, [] (int b)
        {
            if (b == 0) return bell (1000.0, 4.0, 1.0, eq::Lane::Mid);
            if (b == 1) return bell (1000.0, 2.0, 1.0, eq::Lane::Side);
            return eq::BandParams {};
        });
        check (nearEq (ts.compositeDbAxis (1000.0, eq::Axis::Mid),  4.0, 5e-3), "opus: Mid axis folds the Mid lane (axi=3)");
        check (nearEq (ts.compositeDbAxis (1000.0, eq::Axis::Side), 2.0, 5e-3), "opus: Side axis folds the Side lane (axi=4)");
        check (nearEq (ts.compositeDbAxis (1000.0, eq::Axis::Left), 0.0, 1e-9), "opus: Left axis sees neither M nor S lane");
        check (ts.design (0, (int) eq::Lane::Mid).n >= 1, "opus: design() exposes the designed lane sections");
    }
